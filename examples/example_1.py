...
# Provide an example list of tokens we want to match.
match_example = [doc[i] for i in [0, 1, 3]]  # [We, introduce, methods]

# Note these tokens must be constitute a fully connected graph. Otherwise, spacy-pattern-builder will raise a TokensNotFullyConnectedError. You can get the smallest connected subgraph that includes your tokens like:
from spacy_pattern_builder import util
connected_subgraph = util.smallest_connected_subgraph(match_example, doc)
assert match_example == connected_subgraph

# Specify the token attributes / features to use
feature_dict = {  # This is equal the default feature_dict
    'DEP': 'dep_',
    'TAG': 'tag_'
}
...
# Create a matcher with the newly generated pattern
